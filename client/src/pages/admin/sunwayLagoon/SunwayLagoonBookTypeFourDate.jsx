  ...
  const [isLoading, setIsLoading] = useState(false)
    ...
    const disabledDays = blockedDates?.map((dates) => new Date(dates.blockDates))
      ...
      const addBlockDate = async () => {
        try {
          setIsLoading(true)
            const {data} = await axios.post('/api/v1/dates-manage/block-dates', {blockDates: dateToString, service:"sunway-lagoon", type: "bookTypeFour"})
        ...
        const getBlockDates = async () => {
          try {
            const {data} = await axios.get(`/api/v1/dates-manage/block-dates?service=sunway-lagoon&type=bookTypeFour`)
